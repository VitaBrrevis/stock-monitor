...
# Global variables to track current file names and creation times
current_files = {
    '3hour': {'file': None, 'created_at': None},
    '24hour': {'file': None, 'created_at': None}
}
...
def get_scheduler_filename(scheduler_type):
    """Get or create filename for specific scheduler type."""
    with file_lock:
        current_time = datetime.now()

        # Check if we need a new file (every 7 days or if no file exists)
        if (current_files[scheduler_type]['file'] is None or
                current_files[scheduler_type]['created_at'] is None or
                (current_time - current_files[scheduler_type]['created_at']).total_seconds() >= 604800):  # 7 days = 604800 seconds

            # Create new file
            timestamp = current_time.strftime('%Y-%m-%d_%H-%M-%S')
            filename = os.path.join(DATA_DIR, f'stock_data_{scheduler_type}_{timestamp}.csv')

            # Create file with headers and column descriptions
            ...
            current_files[scheduler_type]['created_at'] = current_time
...
def save_stock_data_batch(all_product_details, scheduler_type):
    """Save all collected stock data to scheduler-specific CSV file."""
    ...
    # Add scraping session separator
    ...
    logging.info(f"Saved {len(rows)} product records to {scheduler_type} CSV file: {csv_file}")
...
def main():
    ...
    logging.info("New files will be created every 7 days for each scheduler")
